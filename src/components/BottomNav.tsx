        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className="flex flex-col items-center justify-center flex-1 h-full transition-all duration-200 cursor-pointer relative"
              style={{ color: isActive ? 'var(--primary)' : 'var(--on-surface-variant)' }}
            >
              {isActive && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-12 h-1 rounded-b-full" style={{ backgroundColor: 'var(--primary)' }} />
              )}
